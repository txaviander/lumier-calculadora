...
export function DashboardSidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 z-50 h-full w-[260px] bg-gray-50 border-r border-gray-200 flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 ${
      ...
      >
          ...
          <div className="flex items-center justify-between">
            <Link href="/" className="flex items-center gap-2">
              <h1 className="text-xl font-semibold tracking-wide text-gray-900">
                LUMIER <span className="font-normal text-gray-500">BRAIN</span>
              </h1>
            </Link>
            <button
              onClick={onClose}
        ...
        <div className="p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <span className="inline-block px-2 py-0.5 text-[10px] font-medium text-gray-500 bg-gray-200 rounded-full mt-0.5">
              Dirección
            </span>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-6">
          {menuSections.map((section) => (
            <div key={section.title}>
              <p className="px-3 text-[10px] font-semibold tracking-wider text-gray-400 mb-2">
                {section.title}
              </p>
                    ...
                    return (
                      <li key={item.name}>
                        <a
                          ...
                          onClick={onClose}
                          className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 group text-gray-600 hover:bg-gray-900 hover:text-white"
                        >
                          {item.icon}
                          <span className="flex-1">{item.name}</span>
                          <svg
